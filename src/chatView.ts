...
export class ChatView {
    ...
    public static readonly viewType = 'qcodeChat';
    ...
    private async _handleSendMessage(text: string, model: string) {
        try {
            ...
            // Call AI provider
            const response = await this._aiProvider.sendMessage(fullPrompt, model);
    ...
    private async _handleLoadModels() {
        try {
            ...
            this._panel.webview.postMessage({
                command: 'showError',
                content: `Failed to load models: ${error.message || 'Unknown error'}`
            });
        }
    }
