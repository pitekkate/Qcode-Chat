...
\`\`\``
    ...
    static async startTutorial(tutorialId: string) {
        ...
        // Tampilkan tutorial di webview panel
        ...
        panel.webview.html = this.getTutorialHtml(tutorial);
        
        panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                case 'nextStep':
                    await this.showNextStep(panel, tutorial, message.stepIndex);
                    break;
                case 'prevStep':
                    await this.showPrevStep(panel, tutorial, message.stepIndex);
                    break;
                case 'completeTutorial':
                    await this.completeTutorial(panel, tutorial);
                    break;
            }
        });
    }

    private static getTutorialHtml(tutorial: any) {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        .tip-box {
            background-color: var(--vscode-welcomePage-progressBackground);
...
<body>
    <div class="tutorial-container">
        ...
        <div class="progress">
            <span id="progressText">Step 1 of ${tutorial.steps.length}</span>
        </div>
        
        <div class="step-container" id="stepContainer">
            ${this.getStepHtml(tutorial.steps[0], 0)}
        </div>
        
        <div class="navigation">
            <button class="nav-btn" id="prevBtn" disabled onclick="prevStep()">Previous</button>
    ...
    <script>
        ...
        function nextStep() {
            if (currentStep < totalSteps - 1) {
                currentStep++;
                updateStep();
                vscode.postMessage({ command: 'nextStep', stepIndex: currentStep });
            } else {
                completeTutorial();
            }
        }
        ...
        function prevStep() {
            if (currentStep > 0) {
                currentStep--;
                updateStep();
                vscode.postMessage({ command: 'prevStep', stepIndex: currentStep });
            }
        }
        
        function completeTutorial() {
            vscode.postMessage({ command: 'completeTutorial' });
        ...
        function updateStep() {
            ...
            stepContainer.innerHTML = ${JSON.stringify(this.getStepHtml(tutorial.steps[0], 0))};
    ...
    private static getStepHtml(step: any, index: number) {
        return `
    ...
    private static async showNextStep(panel: vscode.WebviewPanel, tutorial: any, stepIndex: number) {
        // Logic untuk menampilkan step berikutnya
    }

    private static async showPrevStep(panel: vscode.WebviewPanel, tutorial: any, stepIndex: number) {
        // Logic untuk menampilkan step sebelumnya
    }

    private static async completeTutorial(panel: vscode.WebviewPanel, tutorial: any) {
        panel.dispose();
